...
class GameViewController: ViewController {
    ...
    var postionX: [CGFloat] = [0.0,0.0,0.0]
    ...
    @objc func up(){
        for i in 0..<3{
            if postionX[i] > width || postionX[i] < 0{
                dx[i] = dx[i] * (-1)
            }
            postionX[i] += dx[i]
        }
        imgView1.center.x = postionX[0]
        imgView2.center.x = postionX[1]
    }
    ...
    @IBAction func stop(){
        ...
        for i in 0..<3{
            score = score - abs(Int(width/2 - postionX[i]))*2
        }
        ...
        if score > highScore1{
            ...
            //defaults.set(highScore1, forKey: "score1")
            defaults.set(score, forKey: "score2")
            defaults.set(highScore2, forKey: "score3")
        }else if score > highScore3{
            // defaults.set(highScore1, forKey: "score1")
            //defaults.set(highScore2, forKey: "score2")
    ...
    @IBAction func retry(){
        score = 1000
        postionX = [width/2,width/2,width/2]
